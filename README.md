...
The scenario is a set of *cycles*: A **cycle** is a repeating set of events in the scenario. 
...
### 2a. Input.
<hr/>
In this section you will learn what data program reads from the user.
<hr/>
...
<hr/>
In this section you will learn what data program returns to the user and in what form.
<hr/>
...
- $\theta$ - The angle between the horizontal and the surface.
...
Static surface (on which the block is laying) is hereinafter referred to as the *plane*.\
Static, perfectly elastic surface, normal to the plane (on scheme 4. at the beginning of the plane)
...
Between the end of any $k$-th cycle and the start of the $(k+1)$-th cycle
one and only event is the perfectly elastic collision between the point and the wall.
...
As the *wall* is the static object ($E=0$), the kinetic energy of the *point* is equal at the end of $k$-th cycle
and start of $(k+1)$-th _cycle_. 
Therefore, there is a recurrence relation between adjacent _cycles_ that can be expressed as[^2]:
...
However, our model is not isolated due to the non-conservative force $\vec{T}$[^4]. 
After including the $W_T$ (work of friction)[^5] 
in the conservation of energy formula we obtain:
...
Using formulas $(1)$ - $(6)$ we can obtain full model data for the $(k+1)$-th cycle
knowing only constants and the $k$-th end velocity.

There is a corner case when $\vec{T}$ is bigger than the horizontal $\vec{Q}$ value. 
In this situation, in 2nd stage $\vec{T}$ changes to static friction ($\vec{T_s}$)
...
- The program uses $(1)$ - $(6)$ recurrence formulas to prepare the theoretical model of scenario. 
The program treats start a velocity given by the user as the fictional $0$th end velocity.
- The program stops counting cycles when the $n$-th end velocity is close to zero. 
- The program checks *not full cycle* occurrence based on the $(7)$ formula.
- The program predicts the simulation behavior based on the theoretical model. 
If the theoretical model has ${n}$ cycles then the simulation waits for ${n}$ cycles before ending.
If the theoretical model has not full cycle then the simulation is also expecting not full cycle.
- Errors are prepared with data from the theoretical model as a true values.

### 4. Simulation's details.
<hr>
In this section you can learn some technical facts and issues with the simulation in the program.
<hr>

The simulation run on the *[Pymunk](https://www.pymunk.org)* physics engine and the *[Pygame](https://www.pygame.org)* graphical interface.
...
You can observe that with every simulation's cycle, results get less precise.

### 5. A development environment setup.
<hr>
...
#### The first setup.
The project is using the *[Pipenv](https://pipenv.pypa.io/en/latest/)* Python's module as a package manager, 
...
Remember to follow Terms and Conditions in the LICENSE file.

### 6. License
<hr>
This section describes licensing of the software.
<hr>
This software is licensed under the *[Apache 2.0](http://www.apache.org/licenses/LICENSE-2.0)* license.
...
You can find full license text in the LICENSE file alongside with the notice in NOTICE file.
